...
func index(w http.ResponseWriter, req *http.Request) {
	...
	if chunk_ID == "2" {
		fmt.Fprintf(w, "http://192.168.1.5:8092/fasta?id=2")

	} else if chunk_ID == "3" {
		fmt.Fprintf(w, "http://192.168.1.5:8092/fasta?id=3")
	}
...
func get(w http.ResponseWriter, req *http.Request) {
	...
	// read file line by line
	if chunk_ID == "2" {
		Input_fileName = "slave2.fasta"

	} else if chunk_ID == "3" {
		Input_fileName = "slave3_copy.fasta"
	}
...
func post(w http.ResponseWriter, req *http.Request) {
	...
	if string(b[0]) == "2" {
		err = ioutil.WriteFile("slave2.fasta", b[1:], 0644)
	} else if string(b[0]) == "3" {
		err = ioutil.WriteFile("slave3_copy.fasta", b[1:], 0644)
	}
	if err != nil {
		log.Fatal(err)
	}
...
func handle_bases_count(w http.ResponseWriter, req *http.Request) {
	...
	my_count_Result_location := "http://192.168.1.5:8092/countResult?id=" + map_chunk_ID
